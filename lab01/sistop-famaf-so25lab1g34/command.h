...
/*
 * Obtiene el comando simple en la posición `index` del pipeline.
 *   self: pipeline del cual obtener el comando.
 *   index: posición (0..pipeline_length(self)-1) del comando.
 * Returns: puntero al scommand en esa posición (no copia, no se apropia).
 * Requires: self != NULL && index < pipeline_length(self)
 */
