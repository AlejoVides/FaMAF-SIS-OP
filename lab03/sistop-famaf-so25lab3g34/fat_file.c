...
/* Writes zeros to the given cluster in the FAT data area.
 * This effectively clears any previous data stored in that cluster,
 * preventing leftover (garbage) content from appearing in new files
 * or directories that reuse it.
 *
 * Returns 0 on success, or -1 on error.
 */
static int clear_cluster(fat_table table, u32 cluster) {
    DEBUG("Clearing cluster for new directory");
    off_t offset = fat_table_cluster_offset(table, cluster);
    u32 bytes_per_cluster = fat_table_bytes_per_cluster(table);
    void *zeros = calloc(1, bytes_per_cluster);
    if (zeros == NULL) {
        return -1;
    }

    ssize_t written = pwrite(table->fd, zeros, bytes_per_cluster, offset);
    free(zeros);
    if (written != bytes_per_cluster) {
        return -1;
    }
    return 0;
}
...
fat_file fat_file_init(fat_table table, bool is_dir, char *filepath) {
    ...
    if (fat_table_is_cluster_used(table, start_cluster)) {
        DEBUG("First free cluster selected for new file is in use!");
        errno = ENOSPC;
        fat_file_destroy(new_file);
        return NULL;
    }
    new_file->dentry = fat_file_init_direntry(is_dir, filepath, start_cluster);
    if (new_file->dentry == NULL) {
        DEBUG("Error creating dentry for new file");
        fat_file_destroy(new_file);
        return NULL;
    }
    ...
    int error = fat_table_set_next_cluster(
        table, start_cluster, FAT_CLUSTER_END_OF_CHAIN_MAX);
    if (error != 0) {
        fat_file_destroy(new_file);
        return NULL;
    }

    // If creating a directory, initialize its cluster to zero to avoid leftover
    // data
    if (is_dir && clear_cluster(table, start_cluster) != 0) {
        DEBUG("Failed to clear cluster %u for new file \"%s\"", start_cluster,
              filepath);
        fat_file_destroy(new_file);
        return NULL;
    }
...
/* Writes the @file's directory entry into the @parent's cluster.*/
int write_dir_entry(fat_file parent, fat_file file) {
    // Calculate the starting position of the parent's cluster
    off_t parent_offset =
        fat_table_cluster_offset(parent->table, parent->start_cluster);

    // Check the position of the file's entry in the parent's cluster is valid
    ...
    // Calculate the position of the file's directory entry
...
void fat_file_dentry_add_child(fat_file parent, fat_file child) {
    ...
    // FIXME this would be more efficient if we used the first free slot on the
    // cluster.
    child->pos_in_parent = parent->dir.nentries;
 ...
 */
static void parse_dir_entries_in_cluster(u8 *buffer, fat_dir_entry end_ptr,
...
GList *fat_file_read_children(fat_file dir) {
    ...
    // This buffer will contain the content of the directory cluster
    ...
    while (fat_table_cluster_is_valid(cur_cluster)) {
        ...
        parse_dir_entries_in_cluster(buf, end_ptr, dir, &entry_list);
...
void fat_file_truncate(fat_file file, off_t offset, fat_file parent) {
    ...
    int error = fat_table_set_next_cluster(file->table, last_cluster,
    ...
    while (fat_table_cluster_is_valid(last_cluster)) {
        ...
        next_cluster = fat_table_get_next_cluster(file->table, last_cluster);
        error = fat_table_set_next_cluster(file->table, last_cluster, FAT_CLUSTER_FREE);
        if (error != 0) {
            return;
        }
        last_cluster = next_cluster;
 ...
 * Returns 0 if no free cluster can be found, or the FAT table can't be written
...
static u32 next_or_new_cluster(fat_file file, u32 last_cluster) {
    ...
    if (fat_table_cluster_is_EOC(next)) {
        ...
        error = fat_table_set_next_cluster(file->table, last_cluster, next);
        error = fat_table_set_next_cluster(file->table, next, FAT_CLUSTER_END_OF_CHAIN_MAX);
        if (error != 0) {
            return 0;
        }
    }
...
ssize_t fat_file_pwrite(fat_file file, const void *buf, size_t size,
    ...
    while (bytes_remaining > 0) { // There are still bytes to write
        ...
        if (bytes_remaining > 0) {
            cluster = next_or_new_cluster(file, cluster);
            if (cluster == 0 || !fat_table_cluster_is_valid(cluster)) {
                break;
            }
        }
    }
